...
/*   Updated: 2022/06/23 17:06:04 by jforner          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	create_color(int *array, char *parse)
{
	...
	while (++i < 3)
	{
		if (tablen(color) != 3 || str_is_num(color[i])
...
int	options_switch2(t_play *play, char **parse, int dir[6])
{
	...
	if (test == NULL || iw != TEXWIDTH || iw != TEXHEIGHT)
		return (1);
	if (!ft_strncmp(parse[0], "WE", 2) && tablen(parse) == 2 && dir[3]--)
		i = 3;
	else if (!ft_strncmp(parse[0], "NO", 2) && tablen(parse) == 2 && dir[0]--)
		i = 0;
	else if (!ft_strncmp(parse[0], "EA", 2) && tablen(parse) == 2 && dir[2]--)
		i = 2;
	else if (!ft_strncmp(parse[0], "SO", 2) && tablen(parse) == 2 && dir[1]--)
		i = 1;
	else
		return (1);
...
int	options_switch(t_play *play, char **parse)
{
	...
	if (!ft_strncmp(parse[0], "C", ft_strlen(parse[0])) && dir[4])
	{
		if (create_color(&(play->texture->ceil), tjoin(&parse[1])) || --dir[4])
			return (1);
	}
	else if (!ft_strncmp(parse[0], "F", ft_strlen(parse[0])) && dir[5])
	{
		if (create_color(&(play->texture->floor), tjoin(&parse[1])) || --dir[5])
			return (1);
	}
	else if (options_switch2(play, parse, dir))
		return (1);
	return (0);
}
